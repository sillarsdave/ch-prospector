...
if search_btn:
    if not api_key:
        ...
        for c in all_items:
            ...
            # Exclude companies whose accounts filings are all dormant
            # (catches dormant companies registered with active SIC codes)
            if excl_dormant:
                try:
                    auth = "Basic " + b64encode(f"{api_key}:".encode()).decode()
                    fh = requests.get(
                        f"{API_BASE}/company/{num}/filing-history",
                        params={"category": "accounts", "items_per_page": 5},
                        headers={"Authorization": auth}, timeout=10
                    )
                    if fh.status_code == 200:
                        filings = fh.json().get("items", [])
                        if filings and all(
                            "dormant" in f.get("description", "").lower()
                            for f in filings
                        ):
                            continue
                except:
