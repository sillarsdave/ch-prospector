...
def fetch_financials(company_number, api_key):
    ...
    try:
        ...
        try:
            ...
            accountant = ""
            import re as _re
            # Patterns to find accountant firm name
            acct_patterns = [
                r"(?:prepared by|statutory auditors?|reporting accountants?|independent auditors?|audited by|accounts? (?:have been )?prepared by)[:\s]+([A-Z][A-Za-z &,\.]+(?:LLP|Ltd|Limited|LTD|Chartered|Accountants?)?)",
                r"([A-Z][A-Za-z &,\.]{3,50}(?:LLP|Chartered Accountants|Certified Accountants))",
            ]
            for pat in acct_patterns:
                m = _re.search(pat, full_text)
                if m:
                    candidate = m.group(1).strip().rstrip(".,")
                    # Sanity check - ignore if it looks like a company name or address
                    skip_words = ["the company","the directors","companies house","hmrc","limited company"]
                    if len(candidate) > 4 and not any(s in candidate.lower() for s in skip_words):
                        accountant = candidate[:60]
                        break
