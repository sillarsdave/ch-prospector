...
if results:
    ...
    with col_a:
        ...
        try:
            ...
            # Column definitions: (header label, data key, is_link)
            col_defs = [
                ("Score",          "Score",         False),
                ...
                ("SIC",            "SIC",           False),
                ("Incorporated",   "Incorporated",  False),
                ("Age (years)",    "Age",           False),
                ("Total Assets",   "Total Assets",  False),
                ("Net Assets",     "Net Assets",    False),
                ("Fixed Assets",   "Fixed Assets",  False),
                ("Current Assets", "Current Assets",False),
                ("Employees",      "Employees",     False),
                ("Accounts Date",  "Accounts Date", False),
                ("Dir. Appointed", "Dir. Appointed",False),
                ("CH Company",     "CH Link",       True),
                ("Officers",       "_officers",     True),
            ...
            for ci, (label, _, _is_link) in enumerate(col_defs, 1):
            ...
            for rn, (_, row) in enumerate(df.iterrows(), 2):
                ...
                officers_url = f"{row['CH Link']}/officers" if row.get('CH Link') else ""
                for ci, (label, key, is_link) in enumerate(col_defs, 1):
                    if key == "_officers":
                        val = officers_url
                    else:
                        val = row.get(key, "")
                    ...
                    if is_link and val:
                        link_labels = {"CH Link": "Open", "_officers": "Officers", "LinkedIn": "Search"}
                        cell.value = link_labels.get(key, "Open")
                        cell.hyperlink = val
            ...
            # Auto-fit column widths (cap 40, min 8)
            for ci, (label, _, _) in enumerate(col_defs, 1):
