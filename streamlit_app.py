...
with st.sidebar:
    ...
    email_to = st.text_input("Send to", value=gmail_user if email_configured else "", placeholder="your@email.com")
...
if search_btn:
    if not api_key:
        ...
        while t1.is_alive() or t2.is_alive():
            ...
            # Total units of work: dirs + fins (fins count double as they're slower)
            fins_weight = 2 if fetch_financials_flag else 0
            total_work = total * (1 + fins_weight)
            done_work = d + (f * fins_weight)

            # Calculate ETA based on rolling rate over last few seconds
            time_delta = now - last_time[0]
            work_delta = done_work - last_done[0]
            if time_delta >= 2 and work_delta > 0:
                rate = work_delta / time_delta  # units per second
                remaining_work = total_work - done_work
                if rate > 0:
                    # Factor in rate limiting - if paused, add buffer
                    raw_eta = remaining_work / rate
                    if _rate_limiter.paused:
                        raw_eta *= 1.5
                    eta_seconds[0] = raw_eta
            ...
            # Format ETA
            if eta_seconds[0] is not None and done_work > 0:
                eta = int(eta_seconds[0])
                if eta >= 3600:
                    ...
                    eta_str = f"{eta}s"
                eta_display = f"&nbsp;&nbsp; ⏱ **Est. remaining: {eta_str}**"
            elif done_work == 0:
            ...
            overall = 30 + int((done_work / max(total_work, 1)) * 55)
        ...
        for c in all_items:
            ...
            # Exclude companies whose only filings are dormant
            # fetch_financials already skips dormant filings, so if accounts_date
            # is empty and the company status isn't obviously active, skip it.
            if excl_dormant and not fin.get("accounts_date","") and not fin.get("net_assets","") and not fin.get("total_assets",""):
                # No financial data at all - likely dormant, skip
                continue
